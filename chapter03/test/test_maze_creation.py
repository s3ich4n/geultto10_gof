...
def test_standard_builder(mage_built_by_standard):
    room1 = mage_built_by_standard.room_no(1)
    room2 = mage_built_by_standard.room_no(2)

    assert isinstance(room1, Room)
    assert isinstance(room2, Room)


def test_continuing_builder(maze_built_by_continuing):
    rooms, doors = maze_built_by_continuing.get_counts()
    assert rooms == 2
    assert doors == 1
